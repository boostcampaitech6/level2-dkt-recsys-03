...
class Saint(ModelBase):
    ...
    def forward(self, test, question, tag, correct, mask, interaction):
        ...
        test, question, tag, correct, mask, interaction = test.to(float).long(), question.to(float).long(), tag.to(float).long(), correct.to(float).long(), mask.to(float).long(), interaction.to(float).long()
        ...
        # 사실 이렇게 3개로 나눌 필요가 없다
        if self.enc_mask is None or self.enc_mask.size(0) != seq_len:
            self.enc_mask = self.get_mask(seq_len).to(device)

        if self.dec_mask is None or self.dec_mask.size(0) != seq_len:
            self.dec_mask = self.get_mask(seq_len).to(device)

        if self.enc_dec_mask is None or self.enc_dec_mask.size(0) != seq_len:
            self.enc_dec_mask = self.get_mask(seq_len).to(device)
        ...
        # Positional encoding
        embed_enc = torch.tensor(self.pos_encoder(embed_enc), dtype = torch.float32)
        embed_dec = torch.tensor(self.pos_decoder(embed_dec), dtype = torch.float32)

        print("positional")
        print(type(embed_enc))
        print(type(embed_dec))
